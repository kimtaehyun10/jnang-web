...
function mem_add() {
	dataCnt ++;
	var fld = "<div id='m_list"+ dataCnt +"' style='margin:5px;'>성명 : <span style='margin:0px 5px 0 5px;display:inline-block; width:100px; border: none;'><input type='text' name='uname' value='' style='width:100%;'></span> "
		...
		+ "<a onclick='del(\""+ dataCnt +"\");'> - 삭제</a></div>";
...
function send() {
	try {
		...
		for (ii = 0 ; ii < dataCnt; ii++) {
			var data = $("input[name=uname]").eq(ii).val()
					+ "|"+ $("input[name=ubrth]").eq(ii).val()
